(function(){
  ...
  // Fits the whole shell (HUD bar + canvas + joystick bar) inside the real
  // available viewport, so nothing is cut off on desktop or mobile.
  function layoutGame(){
    const vw = window.innerWidth;
    const vh = (window.visualViewport ? window.visualViewport.height : window.innerHeight);
    const outerPad = 20;
    const gapTotal = 16; // gaps between the stacked rows

    const hudH = hudBar.offsetHeight || 0;
    const touchH = touchControlsEl.classList.contains('active') ? (touchControlsEl.offsetHeight || 0) : 0;

    const availW = Math.max(240, vw - outerPad);
    const availH = Math.max(200, vh - outerPad - hudH - touchH - gapTotal);
  ...
  window.addEventListener('orientationchange', layoutGame);
  if(window.visualViewport){ window.visualViewport.addEventListener('resize', layoutGame); }

  const isTouch = ('ontouchstart' in window) || navigator.maxTouchPoints > 0;
  if(isTouch){ document.getElementById('touchControls').classList.add('active'); document.body.classList.add('is-touch'); }
  ...
  const GHOST_COLORS = [
    ...
    { fill:'rgba(150,165,255,0.65)', stroke:'rgba(195,205,255,0.6)', glow:'150,165,255' }, // periwinkle
  ...
  function render(now){
    ...
    // coins — gold, so they read as distinct from the blue player
    for(const c of coins){
      ...
      grad.addColorStop(0.4,'rgba(255,208,90,0.9)');
